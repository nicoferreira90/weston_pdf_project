...
**Status:** CI implemented; hosted verification and tenant design note pending
...
- [ ] A hosted GitHub Actions run confirms both jobs passed and the expected test counts, then the README links its status badge.
- [ ] README explains configuration identity, immutable versions, stable field IDs, retained historical meaning, request validation, stale-version handling, activation, and rollback, and clearly labels the design as unimplemented.
...
- Hosted checkout, runtime setup, dependency installation, and push/pull-request triggers still require the first GitHub Actions run. No hosted success or badge is claimed.
- The tenant configuration README note remains the second deliverable. WES-05's actual working time has not yet been recorded; the 1-hour figure is its budget.
